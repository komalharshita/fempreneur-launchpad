...
const ToolCard = ({ icon: Icon, title, helperText, toolId, fields, buttonLabel }: ToolCardProps) => {
  ...
  return (
    <div className="flex flex-col rounded-2xl bg-card p-6 shadow-card transition-all duration-300 hover:-translate-y-0.5 hover:shadow-card-hover">
      ...
      <div className="flex flex-col gap-3">
        {fields.map((f) => (
          <input
            ...
            className="rounded-xl border border-border bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-ring"
          />
        ))}
      </div>
      ...
      {result && (
        <div className="mt-5 animate-in fade-in-0 slide-in-from-bottom-2 duration-500 rounded-xl border border-border bg-secondary/50 p-4">
