...
const inputClass = "w-full rounded-xl border border-border bg-background px-4 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/50 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/60 hover:border-primary/30";
...
const ToolCard = ({ icon: Icon, title, helperText, toolId, fields, buttonLabel }: ToolCardProps) => {
  ...
  return (
    <div className="card-premium flex h-full flex-col p-6">
      ...
      <p className="mb-5 text-sm text-muted-foreground">{helperText}</p>
      ...
      {result && (
        <div className="mt-5 rounded-xl border border-border/60 bg-secondary/50 p-4 animate-in fade-in-0 slide-in-from-bottom-2 duration-500">
